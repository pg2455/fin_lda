...
class getInfo(object):
  ...
  def getDictionary(self, root):
    ...
    'OnlineSections' : getOnlineSections(root), 'idenitified_content': index_cat, 'taxonomic_class' : getTaxonomicClassifier(root) }
...
def getIndexingSections(root):
  ...
  for i in node:
      d['descriptor'] = [x.text for x in root.xpath('.//*[@type="descriptor" and @class="indexing_service"]')]
      d['online_descriptor'] = [x.text for x in root.xpath('.//*[@type="descriptor" and @class="online_producer"]')]
